...
function oxygen_base64_decode_for_json($content) {
	...
	foreach ($dbt as $debug_item) {
		if( isset($debug_item['function']) && ( $debug_item['function'] == 'render_gutenberg_block' || $debug_item['function'] == 'generate_gutenberg_script' ) ){
			// admin part base64 decoding handled on the JS side
			if (!empty($_GET['oxygen_gutenberg_script'])) {
				return $content;
			}
			// re-usable may use JSON even inside Gutenberg
			global $oxy_reusable_in_progress;
			if ($oxy_reusable_in_progress == true) {
				return $content;
			}
